...
const OnboardingSchema = z.object({
  ...
  education_level: z.string().max(80).optional().nullable(),
  ...
  skill_ids: z.array(z.string().uuid()).max(30),
  // work style
...
export const submitOnboarding = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Ensure department + team entities (admin only can create — first user IS admin if create)
    ...
    const profileRow = {
      ...
      education_level: data.education_level ?? null, field_of_study: data.field_of_study ?? null,
    ...
    // Skills & languages: wipe + reinsert
...
export const getMyProfileStatus = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabase.from("profiles").select("id, onboarding_complete, org_id").eq("id", userId).maybeSingle();
    return { profile: data };
  });
...
export const lookupInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    // Uses anon client; org_invites has a SELECT anon policy.
